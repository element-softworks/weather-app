...
const LocationCard: React.FC<{ location: ILocation }> = ({ location }) => {
	...
	return (
		<>
			<Card>
				<CardActionArea
					onClick={() => {
						navigate('/location/' + encodeURIComponent(location.uuid));
					}}
				>
					<CardContent sx={{ display: 'flex' }}>
						<Box sx={{ flex: '1 0 auto' }}>
							<Typography fontSize="2rem">{data?.location?.name}</Typography>
